...
class LDP:
    '''
    Class representing LDP mechanism
    
    _____________
    
    Attributes:
    ldp_config: yaml config
        yaml config for ldp mechnism
    ldp_mechanism: func
        ldp function
    ldp_name: 
        name of used ldp algo
    _____________
    
    Methods:
        apply(self, data, priv_bud = None)
            Applying ldp mechannism on data
        _no_LDP(self, data, priv_bud=None)
            Identity ldp mech, w/o ldp
        _harmony(self, data, priv_bud=None)
            k-harmony ldp
        _laplace(self, data, priv_bud=None)
            laplace ldp
    '''
    def __init__(self, ldp_config):
        self.ldp_config = ldp_config
        self.ldp_mechanism = None
        if ldp_config.HARMONY.APPLY:
            self.ldp_mechanism = self._harmony
            self.ldp_name = "Harmony"
        elif ldp_config.LAPLACE.APPLY:
            self.ldp_mechanism = self._laplace
            self.ldp_name = "Laplace"
        else:
            self.ldp_mechanism = self._no_LDP
            self.ldp_name = "NO"
        self.test_dict = None
        self.random_ = np.random.RandomState(self.ldp_config.SEED)
        
    def apply(self, data, priv_bud = None):
        return self.ldp_mechanism(data, priv_bud)
    
    def _no_LDP(self, data, priv_bud=None):
        return data
    
    def _harmony(self, data, priv_bud=None):
        k = self.ldp_config.HARMONY.k
        if priv_bud is None:
            priv_bud = self.ldp_config.PRIVACY_BUDGET
            
        flatten_data = data.reshape(1,-1) 
        dimension = flatten_data.shape[1] 
        
        num_users = flatten_data.shape[0] 
        flatten_pertubed_data = np.zeros(flatten_data.shape)
        rand_indexes = self.random_.randint(0, dimension, size = (k, num_users))
        flatten_data_max, flatten_data_min = flatten_data.max(), flatten_data.min()
        flatten_norm_data = flatten_data
        
        prob_pos = (flatten_norm_data[np.arange(num_users), rand_indexes]  * \
                        (np.exp(priv_bud) - 1) + np.exp(priv_bud) + 1) / 2 / (np.exp(priv_bud) + 1)
        sampled_bernoulli = self.random_.binomial(n=1,p=prob_pos, size =(k, num_users))
        sampled_sign = np.power(-1, sampled_bernoulli + 1)
        flatten_pertubed_data[np.arange(num_users), rand_indexes] = sampled_sign * dimension / k  * \
        (np.exp(priv_bud) + 1) / (np.exp(priv_bud) - 1)

        return flatten_pertubed_data.reshape(data.shape)

    def _laplace(self, data, priv_bud=None):
        if priv_bud is None:
            priv_bud = self.ldp_config.PRIVACY_BUDGET
        sensativity = (data.max() - data.min())
        dimension = data.size
        num_users = data.shape[0]
        
        LDP_noise = self.random_.laplace(0,  dimension * sensativity / priv_bud, 
                                                    size = np.shape(data))
...
def update_global_model(
    ...
    evaluation_callback, iterator, ldp  ### TO DEBUG
):
    """
        To fill in!
    """
    ...
    for _ in iterator(n_iter):
        for factor_name in global_factors_names:
            update_global_factor(
                ...
                gradient_funcs, regularization, gain, ldp
            )
...
def update_global_factor(
    ...
    for user in range(n_users):
        grad += ldp.apply(gradient_update(user))
...
def fit_restricted(
    ...
    evaluation_callback=None, use_eta=False, iterator=None, ldp=None,
